...
void Auswahltool(int gridX, int gridY)
{
    ...
    bool mgateGeklickt = false;
    bool naviGeklickt = false;
    ...
    for (const auto& Gate : GateListe) {
        if (Gate.GridX == gridX && Gate.GridY == gridY) {
            ...
            EndereMausZu = "Hand";
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                ausgewahlterGate = Gate.eindeutigeId;
                mgateGeklickt = true;
            }
        }
    }
    for (const auto& Navi : NaviListe) {
        if (Navi.GridX == gridX && Navi.GridY == gridY) {
            ...
            EndereMausZu = "Hand";
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                ausgewahlterNavi = Navi.eindeutigeId;
                naviGeklickt = true;
            }
        }
    }
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !mgateGeklickt) {
        ausgewahlterGate = 0;
    }
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !naviGeklickt) {
        ausgewahlterNavi = 0;
    }
}
/*-------------------------------------------------
...
void Loeschentool(Vector2 mausposition) {
    ...
    // Marker
    else if (IstGateVorhanden(MouseGridX, MouseGridY)) {
        EndereMausZu = "Hand";
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            GateLöschen(MouseGridX, MouseGridY);
            return;
        }
    }
    // Navi
